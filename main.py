...
xgboost_featureEngineered_model = load_model('xgboost_model-featureEngineered.pkl')
...
def explain_prediction(probability, input_dict, surname):
    ...
    | Balance          | 0.052786  |
    | Geography_France | 0.044663  |
    | Gender_Female    | 0.045283  |
    | Geography_Spain  | 0.036585  |
    | CreditScore      | 0.030505  |
    | EstimatedSalary  | 0.032055  |
    | HasCrCard        | 0.031940  |
    | Tenure           | 0.030054  |
...
if selected_customer_option:
    ...
    # Get the selected customer row as a DataFrame
    selected_customer = df.loc[df["CustomerId"] == selected_customer_id]

    # Make sure to get the first row (iloc[0]) since loc returns a DataFrame
    if not selected_customer.empty:
        selected_customer = selected_customer.iloc[0]
